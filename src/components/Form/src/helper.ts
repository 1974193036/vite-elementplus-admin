...
import { PlaceholderMoel } from './type'
...
export const setComponentProps = (item: FormSchema): Recordable => {
  ...
  const datePickerCom = ['DatePicker']

  let componentProps: Recordable = {}

  if (notNeedClearable.includes(item.component as string)) {
    componentProps = { ...item.componentProps }
  } else if (datePickerCom.includes(item.component as string)) {
    // 日期组件默认设置
    // 根据项目情况自行设置
    componentProps = {
      clearable: false,
